...
class TicketQRGive extends Component {
    constructor(props, context) {
        super(props);
		this.contracts = context.drizzle.contracts;
		this.event = this.contracts['Shelter'].methods.callForHelpDetails.cacheCall(this.props.ticket.returnValues.eventId);
		this.address = null;
		this.state = {
			loading: false,
			loaded: false,
	...
	updateIPFS = () => {
        console.log(this.state.loaded, this.state.loading,typeof this.props.contracts['Shelter'].callForHelpDetails[this.event])
		if (this.state.loaded === false && this.state.loading === false && typeof this.props.contracts['Shelter'].callForHelpDetails[this.event] !== 'undefined') {
			this.setState({
				loading: true
			}, () => {
...
dload =(id,title)=>{
	...
	render() {
		console.log('event ID',this.props.ticket.returnValues)
			...
			let card_body;
				...
				card_body =  <div className="ticket-wrap"><div class={border} id={'ticket-'+this.props.ticket.transactionHash}>					
						...
						<h5 className="ticket-shelter">SHelteR</h5>
										...
										<h1 className="ticket-organizer">Organized By: {ticket_data.receiver}</h1>

									</div>
									<div className="ticket-item"><p>Item</p> <h1>{ticket_data.item} - {ticket_data.committed}x  </h1></div>
									...
									<QRCode 
    									...
    									y:75,
    									excavate: false,}}/></p>	
								<h1 className="ticket-quantity">{ticket_data.committed}x  </h1>
...
const AppContainer = drizzleConnect(TicketQRGive, mapStateToProps);
