...
class ReminderReturnTake extends Component {
    constructor(props) {
        super(props);
...
render(){
    ...
    <div className="dashboard-events-list">
              {this.state.commited.map((pledged,index)=>(<h4 className="col-md-12 small" key={index}><strong className="gold">{pledged.returnValues.received} {pledged.returnValues.item} </strong> was borrowed by <strong>{pledged.returnValues.receiver}</strong> on <span className="date-right small">{this.parseDate(pledged.returnValues.date)}</span> & should be returned to <strong onClick={()=>this.friendlyUrl(pledged.returnValues.sender,pledged.returnValues.tookFrom)}>{pledged.returnValues.sender}</strong> on, or before <span className="date-right small">{this.parseDate(pledged.returnValues.endDate)}</span></h4>
                ))}
...
export default ReminderReturnTake;
