...
class ReminderPledge extends Component {
    constructor(props) {
        super(props);
...
render(){
    ...
    <div className="dashboard-events-list">
              {this.state.commited.map((pledged,index)=>(<h4 className="col-md-12 small" key={index}><strong className="gold">{pledged.returnValues.committed} {pledged.returnValues.item} </strong> was pledged to <strong onClick={()=>this.friendlyUrl(pledged.returnValues.receiver,pledged.returnValues.pledgeTo)}>{pledged.returnValues.receiver}</strong> on <span className="date-right small">{this.parseDate(pledged.returnValues.date)}</span> & should be returned to <strong>{pledged.returnValues.sender}</strong> on, or before <span className="date-right small"> {this.parseDate(pledged.returnValues.endDate)}</span></h4>
                ))}
